...
export class RolesController {
  ...
  async updateUserRole(
    ...
    if (
      !this.roleValidationService.validateRequiredRoles(currentUser, [
        UserRole.ADMIN,
      ])
    ) {
      throw new ForbiddenException(
    ...
    if (
      !this.roleValidationService.validateRoleTransition(
        targetUser,
        updateRoleDto.role,
      )
    ) {
      throw new ForbiddenException(
        "Cette transition de rôle n'est pas autorisée",
      );
    }
