...
describe('Migration InitialSchema', () => {
  it('devrait créer et supprimer le schéma correctement', async () => {
    ...
    expect(querySpy).toHaveBeenCalledWith(expect.stringContaining('CREATE TYPE'));
    expect(querySpy).toHaveBeenCalledWith(expect.stringContaining('user_role_enum'));
    expect(querySpy).toHaveBeenCalledWith(expect.stringContaining('student_level_enum'));

    // Vérification des appels pour la création des tables
    expect(querySpy).toHaveBeenCalledWith(expect.stringContaining('CREATE TABLE'));
    ...
    expect(querySpy).toHaveBeenCalledWith(expect.stringContaining('instructors'));
    ...
    expect(querySpy).toHaveBeenCalledWith(expect.stringContaining('course_progress'));
    expect(querySpy).toHaveBeenCalledWith(expect.stringContaining('student_courses'));

    // Vérification des appels pour la création des contraintes
    expect(querySpy).toHaveBeenCalledWith(expect.stringContaining('FOREIGN KEY'));
    expect(querySpy).toHaveBeenCalledWith(expect.stringContaining('PRIMARY KEY'));
    expect(querySpy).toHaveBeenCalledWith(expect.stringContaining('UNIQUE'));

    // Vérification des appels pour la création des index
    expect(querySpy).toHaveBeenCalledWith(expect.stringContaining('CREATE INDEX'));
    expect(querySpy).toHaveBeenCalledWith(expect.stringContaining('GIN'));

    // Vérification des appels pour la création des triggers
    expect(querySpy).toHaveBeenCalledWith(expect.stringContaining('CREATE TRIGGER'));
    expect(querySpy).toHaveBeenCalledWith(expect.stringContaining('update_updated_at_column'));
